...
This wouldn't be the same as creating e.g. a `location` post, or a specialized app creating an `episode` post, and making it public. You could (and possibly would) of course do that as well. But those posts would only be read by users and apps who have subscribed to those post types. `share` is what you'd use when you want to push something to someone else (or the whole world).
